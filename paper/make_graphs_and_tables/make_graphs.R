#### Preamble ####
# Purpose: Wrangle the large dataset into graphs and summary statistics
# Author: Lindsay Katz
# Date: 6 November 2025
# Contact: rohan.alexander@utoronto.ca
# License: MIT
# Pre-requisites: Ideally download the data and save it into the data folder.
...
knitr::opts_chunk$set(dev = "ragg_png") # Need this for some reason to make hrbrthemes work

# Conditional import depending on whether the full dataset is loaded
if (file.exists(here("data/corpus_1998_to_2025.parquet"))) {
  corpus <- read_parquet(here("data/corpus_1998_to_2025.parquet"))
} else {
  corpus <- read_parquet(here("data/sample_corpus_1998_to_2025.parquet"))
}
...
parl_dates <- 
  getParliaments() |>
  select(PID, Name, DateOpening, ParliamentEnd) |>
...
women_mps_stats <- 
  read_csv(here("data/women_mps_stats.csv"), show_col_types = F) |>
...
women_mps_stats_plot <- 
  left_join(women_mps_stats, n_interject_parliament, by=c("Parliament","gender")) |>
...
model_df <- 
  read_csv("../model/model_input.csv", show_col_types = F) |>
